        ...
        <div className={`w-full space-y-3 ${className}`}>
            ...
            {value && (
                <div className={`preview-container ${imageLoaded ? 'loaded' : ''}`}>
                    <div className="relative bg-muted rounded-xl p-3 transition-all duration-300">
                        <div className="flex justify-center">
                            <div className="relative">
                                <img
                                    src={value}
                                    alt="Превью"
                                    className={`preview-image w-32 h-32 rounded-lg object-cover border-2 border-border transition-all duration-500 ${
                                        imageLoaded ? 'loaded' : ''
                                    }`}
                    ...
                    {isUploading ? (
                        <>
                            <div className="upload-content flex flex-col items-center">
                                ...
                                <p className="text-sm text-muted-foreground mt-2">Загружаем изображение...</p>
                            </div>
                            <div className="upload-progress"></div>
                        </>
                    ) : (
                        <div className="upload-content flex flex-col items-center">
                            <Upload className="h-8 w-8 text-muted-foreground transition-transform duration-200"/>
                            <p className="text-sm text-muted-foreground mt-2">
                                <span className="text-blue-500 font-medium">Выберите файл</span> или перетащите сюда
                            </p>
                            <p className="text-xs text-muted-foreground mt-1">До 10MB • JPG, PNG, GIF, WebP</p>
                        </div>
                    )}
                </div>
            )}
